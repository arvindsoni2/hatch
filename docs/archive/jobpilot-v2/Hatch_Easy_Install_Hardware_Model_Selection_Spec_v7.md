...
1. UI collects provider choice and non-secret metadata, for example provider name, base URL label, and selected model label.
2. UI may collect the API key in a password field and submit it once over the existing local authenticated backend session. Do not echo the key back to the UI.
3. Backend writes provider metadata only to `${HATCH_HOME}/config/ai_setup_intent.json`.
4. Backend writes secrets only to `${HATCH_HOME}/config/secrets.env`, mounted as `/hatch-home/config/secrets.env`.
5. `secrets.env` must use provider-specific environment variable names already supported by the app where possible, for example `OPENAI_API_KEY`, `GROQ_API_KEY`, `GEMINI_API_KEY`, or equivalent existing repo names.
6. Installer/CLI must create `${HATCH_HOME}/config` with user-only permissions where the platform allows it. Backend should best-effort preserve restrictive permissions, but the host CLI is responsible for warning if permissions are too broad.
7. API responses and doctor/status output may report only `configured: true|false`, provider name, and last validation status. They must never return secret values or secret-length hints.
8. `hatch apply-ai-config` validates that required secret names are present before writing cloud `ai_runtime.json`. It must not print secret values.
9. Services receive secrets through the easy-install compose env-file or existing environment mechanism. Do not copy secrets into Docker labels, command lines, logs, `ai_setup_intent.json`, or `ai_runtime.json`.
10. Secret removal must be explicit, for example `hatch secrets unset <provider>`, or through a UI action that asks for confirmation and then removes only the relevant provider key from `secrets.env`.
