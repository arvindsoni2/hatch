...
### 0.1 Owner decisions to unblock Codex implementation

These decisions override any ambiguous wording elsewhere in this document.

1. **PR execution scope:** Treat this document as a multi-PR roadmap. Start with **PR 1 only** unless the owner explicitly asks for PR 2/3/4/5. Do not implement the whole document in one PR.
2. **Existing setup API:** Preserve and adapt existing setup primitives first: `/api/setup/status`, `/api/setup/hardware`, `/api/setup/cloud-provider`, `/api/setup/skip-ai`, and existing model endpoints. Do not introduce breaking migrations toward `/api/system/probe/*`. New reset endpoints may be added under `/api/setup/reset/*`. New capability status should also live under `/api/setup/*` unless the repo already has a better established namespace.
3. **Password setup:** Onboarding password setup must use the existing local app-lock model/services. Do not introduce a separate user password/auth model in this spec. Existing users with an app-lock password should not be forced through duplicate setup.
4. **Reset and secrets:** Reset/new onboarding/demo reset must preserve host secrets by default, including `api_keys.env`. Deleting secrets requires an explicit destructive option such as `--delete-secrets` or a clearly labelled factory-reset option. PR 1 should align CLI/script behaviour with this rule.
5. **AI mode naming:** `Basic` is a user-facing label for the existing no-AI/not-configured runtime state. Do not replace the existing runtime enum in PR 1. Persist using current states such as `not_configured | cloud | local | custom`; render `not_configured` as `Basic / use Hatch now, set up AI later`.
6. **Provider IDs:** Keep `google_genai` as the canonical stored provider id for Google Gemini because the current frontend/CLI already use it. `google_gemini` may be accepted as a backward/forward-compatible alias, but storage and secrets should canonicalize to `google_genai`. Add `openrouter` as the canonical OpenRouter id.
7. **Watchlist location:** Put Company Watchlist under **Applications** as **Watched companies** for the first implementation, because it feeds the Applications board and Today. It can still be described as Scout-powered in copy/activity logs. Do not add a new top-level navigation item in the first Watchlist PR unless the existing layout strongly supports it.
8. **Scheduled scans:** For PR 3, implement manual scan plus scheduled daily/weekly scans. Reuse an existing scheduler/agent-run mechanism if present; otherwise add only a minimal lightweight scheduler with rate limits, safe defaults, and no heavyweight dependency. This is not part of PR 1.
9. **Question Bank AI scope:** First Question Bank PR should include manual CRUD, tagging, search/filter, and save-from-interview-prep. AI story suggestions or AI-generated answers are deferred unless they can be implemented behind an existing capability gate with minimal risk.
10. **PDF export strategy:** PDF export must be capability-gated. Use an existing converter/browser path if one already exists. Do not add heavyweight PDF dependencies to the lightweight install path. If no safe converter exists, show an unavailable state with a clear fallback and implement the adapter boundary for later enablement.
11. **Spec hygiene:** This revision removes the extra closing code fence from the previous markdown.
...
- Set onboarding step.
- Set AI mode through UI choices `Basic | Local AI | Cloud AI`, mapped to existing runtime states such as `not_configured | local | cloud | custom`.
...
# Preserve/adapt existing setup endpoints first
GET  /api/setup/hardware          # latest/detected hardware status
POST /api/setup/hardware          # run/refresh hardware probe where supported
GET  /api/setup/capabilities      # optional capability status endpoint
...
hatch probe
hatch ai use basic  # UI label; maps to existing not_configured/no-AI runtime state
...
- User can choose Basic/Local AI/Cloud AI from onboarding without CLI; Basic maps to the existing no-AI/not-configured runtime state.
...
type AiProviderId =
  ...
  | 'openai'
  | 'anthropic'
  | 'google_genai'      // canonical stored id; UI label is Google Gemini
  | 'google_gemini'    // accepted alias only; canonicalize to google_genai
  | 'openrouter';
```
...
Add the first implementation under Applications:

```text
Applications -> Watched companies
```

Copy/activity logs may describe it as Scout-powered, but the first PR should not add a new top-level navigation item.
...
- Select source type.
- Manual scan.
- Manual scan plus scheduled daily/weekly scans. Reuse an existing scheduler/agent-run mechanism if present; otherwise add only a minimal lightweight scheduler with rate limits and safe defaults.
...
Applications -> Watched companies page:
...
```text
Implement PR 1 only from docs/Hatch_Revised_Product_Gaps_Control_Plane_Watchlist_QBank_OpenRouter_PDF_Spec_v2_Codex_Unblocked.md.

Treat PR 2 through PR 5 as roadmap scope, not current implementation scope, unless the owner explicitly asks for the next PR.
...
- Add manual scan and daily/weekly scheduled scan; reuse an existing scheduler if present, otherwise add a minimal lightweight scheduler with rate limits and no heavyweight dependency.
...
PR 5:
- Add optional PDF export/preview for generated CV packages, capability-gated and using an existing converter/browser path only if present.
...
Do not implement MCP integration. Do not implement autonomous job application submission. Do not require Scrapling or PDF conversion tooling in lightweight install without capability gating. Do not add heavyweight PDF dependencies to the default install. Add unit, integration, and Playwright tests for all changed flows.
```
