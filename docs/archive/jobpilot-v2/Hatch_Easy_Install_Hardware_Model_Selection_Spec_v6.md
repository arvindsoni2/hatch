...
GPU detection is display-only in v1. Model recommendations must be based on OS/architecture support, available RAM, and model storage capacity only. Do not promote or demote catalogue entries because a GPU is detected unless a later PR explicitly implements GPU-aware inference and tests it across supported platforms. The UI may show a note such as “GPU detected, but v1 recommendations are RAM/disk based.”
...
- Secrets come from existing `.env`/environment handling or the repo’s existing secret pattern.
