...
**The user also picks a coach modality up front — text, voice, or video (§14).** The chosen modality selects the capture path and which perception layers run, and determines which rubric dimensions can be populated. Text and voice are in scope now; video is the opt-in Phase D extension of the same selector.
...
- `coach_mode` — `text | voice | video`; the modality the user chose for the session (§14). Drives capture path, which perception layers run, and which rubric dimensions appear. Maps onto `SessionRecording.recording_type`.
...
- [ ] **Coach modality selector (text / voice) in `components/coach/`** — choose at session start, with a default in settings/profile (§14). Show per-mode requirements (mic for voice) and degrade gracefully when a capability is missing. Built so Phase D can add a `video` option without restructuring.
  - TESTS (Vitest/RTL): selecting `text` posts to `submit-answer`; selecting `voice` posts to `submit-audio`; unsupported mode is disabled with a reason.
  - Suggested commit: `feat(coach): coach modality selector (text/voice)`
...
- [ ] Add the **`video` option to the coach modality selector** (§14), shown only when `perception.face.enabled` and a webcam are present.
...
## 13. Implementation decisions (Phase A)

### 13.1 Audio transport — Option A (multipart endpoint)
- Add `POST /api/coach/sessions/{id}/submit-audio` accepting `multipart/form-data`: audio blob + `question_id`. Keep `submit-answer` (text/JSON) intact as the text-mode and low-power Web Speech path.
- Endpoint: validate content-type + enforce a size cap; save to `data/recordings/{session_id}/{question_id}.{ext}`; set `SessionRecording.audio_uri` and `recording_type`; return **202 + job_id** (same contract as the text path).
- The async job points faster-whisper at the saved file → transcript + word timestamps → writes `speech_metrics`; the rest of the pipeline continues unchanged.
- `recording_type` is the discriminator: `text` (Web Speech / typed) | `audio` (now) | `video` (Phase D). **No migration** — `audio_uri`, `recording_type`, `speech_metrics` already exist on `SessionRecording`.
- Rationale: base64-in-JSON inflates payloads ~33% and is awkward for multi-MB files; multipart is idiomatic and `UploadFile` handles it cleanly. ffmpeg is already in the image, so webm/ogg/m4a/wav all decode — do not constrain the client codec.

### 13.2 Dependency placement — separate file, default-installed, lazy-imported
- Put the perception stack in `requirements-perception.txt` (faster-whisper now; audeering/emotion2vec, Piper later). Keep base `requirements.txt` lean for discovery/scoring/tailoring-only users.
- The default Dockerfile installs `requirements-perception.txt` in its own layer so the coach works out of the box. `perception_factory.py` lazy-imports and raises `PerceptionNotAvailableError`, so slim / non-Docker installs degrade gracefully instead of failing to boot.
- **Models are runtime downloads (~hundreds of MB on first use).** Point faster-whisper's `download_root` (or `HF_HOME`) at a **mounted volume** and add that volume to `docker-compose` so models persist across `--build` rebuilds.

### 13.3 Phase A scope this session — tasks 1–3 only
- Implement transcriber service, delivery metrics from timestamps, and locale fillers. All three are pure, fixture-testable units with zero API/frontend surface — one clean, reviewable commit set.
- Tasks 4–5 (wire `submit-audio` + `useAsyncJob` UX messaging) land in the next pass — now **unblocked** by 13.1, split only for review hygiene.

## 14. Coach modality (user choice)

Meet users where they're comfortable. The user selects one of three modes per session (with a default in settings/profile), progressive-enhancement style:

| Mode | Capture | Server perception | Needs | Rubric dimensions unlocked |
|------|---------|-------------------|-------|----------------------------|
| **Text** | Typed or Web Speech (existing `submit-answer`) | None | Nothing | content, STAR, conciseness, impact (+ fillers if a transcript exists) |
| **Voice** | Audio recording (`submit-audio`) → faster-whisper | ASR + delivery metrics + vocal tone | Mic | the above **+ delivery** (pace/pauses) **+ vocal confidence** |
| **Video** *(Phase D, opt-in)* | Audio + webcam; face analysed **on-device** (MediaPipe) | ASR + tone (audio); face summary from browser | Webcam + explicit consent | the above **+ presence** (eye contact/engagement) |

Principles:
- **Dimensions appear only when the signal exists** (already the rule in §7/§9) — so the rubric naturally scales with the chosen mode; no empty or zeroed dimensions.
- **Graceful degradation:** if the browser/hardware lacks a mic or webcam, that mode is shown disabled with a plain-language reason, never a silent failure.
- **Video is a deliberate, consented choice, never a default** — raw video stays on-device; only the audio and an aggregate face summary reach the server (§8).
- The selector is built once in Phase B (text/voice) and **extended, not rebuilt**, when Phase D adds video.
