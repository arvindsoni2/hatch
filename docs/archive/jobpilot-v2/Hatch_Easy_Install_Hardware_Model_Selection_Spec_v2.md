...
- Store a timestamped probe result locally for diagnostics under `${HATCH_HOME}/probe/hardware_probe.json`.
- The first-run UI may read the latest host-generated probe result through the backend, but the backend must not become the source of truth for host hardware probing.
- If no probe result exists, the UI must show “Hardware not detected yet” and provide the CLI command `hatch probe` or `hatch doctor`.
...
Mount the config directory into the backend container so the backend can safely read/write intent on behalf of the UI:
...
[
  ...
  },
  {
    ...
    "download_url": "https://huggingface.co/Qwen/Qwen3-8B-GGUF/resolve/main/Qwen3-8B-Q5_K_M.gguf",
    "source_revision": "main",
...
- Do not add “latest” or floating unverified model URLs beyond the approved entries.
...
### 23.7 Installing the `hatch` command without administrator access
