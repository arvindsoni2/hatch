...
**v3 clarification update:** Resolves probe/config mount strategy, immutable Hugging Face source revision policy, and low-resource local-AI fallback behaviour.
...
| `download_url_template` | Source URL template using immutable `source_revision` if installer downloads it |
| `source_revision` | Immutable Hugging Face commit hash, never `main` or another floating branch/tag |
| `sha256` | Required integrity check for approved downloads |
...
8. If the 8B primary model is not compatible but the 0.6B model is compatible, offer a degraded low-resource local setup that assigns the 0.6B model to both primary and triage roles with a strong quality warning.
9. If the 0.6B model is also below minimum requirements, local AI is unavailable for beginner mode; offer “configure AI later” or cloud provider setup instead.
...
Low-resource fallback copy:

```text
Your machine does not meet the minimum requirement for the 8B primary model. Hatch can still run a limited local setup using the 0.6B model for both quick triage and primary drafting.

This keeps data local, but quality will be lower. Use it for filtering, rough drafts, and experimentation. For higher-quality CV tailoring and interview prep, choose a larger local model on stronger hardware or configure a cloud provider.
```
...
Initial catalogue is intentionally small. Approve only the two current official Qwen GGUF models already used by Hatch. Both are from official Qwen Hugging Face repositories and both model pages list Apache-2.0 licence.
...
[
  {
    ...
    "source_revision": "<immutable-huggingface-commit-sha>",
...
- Resolve the exact Hugging Face commit hash for each approved file during implementation and set `source_revision` to that immutable commit SHA.
...
- Add a test or validation check that fails if any approved downloadable catalogue entry has `source_revision` equal to `main`, `latest`, empty, or a non-commit placeholder.
...
### 23.7 Low-resource local AI fallback

Do not make local AI unavailable merely because the machine is below the 8B model's 12 GB minimum.

Decision:

- If the 8B primary model is below minimum but the 0.6B model meets minimum requirements, offer a degraded low-resource local setup.
- In that setup, the 0.6B model may serve both `primary` and `triage` roles.
- The UI must show a strong quality warning before confirmation.
- This mode should be labelled as limited/degraded, not balanced or recommended for quality-sensitive output.
- It is acceptable for filtering, rough drafts, and experimentation.
- It is not the preferred mode for detailed CV tailoring, cover-letter quality, or interview preparation.
- If the machine is below the 0.6B model minimum as well, local AI should be unavailable in beginner mode and Hatch should offer cloud AI or configure-later mode.

Example effective config for fallback:

```json
{
  "ai_mode": "local",
  "quality_mode": "degraded_low_resource",
  "primary_model_id": "qwen3-0.6b-q8-triage",
  "triage_model_id": "qwen3-0.6b-q8-triage",
  "warnings": [
    "This low-resource setup uses the same 0.6B model for both primary and triage tasks. Quality will be lower than the 8B setup."
  ]
}
```
