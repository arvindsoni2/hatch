...
**v4 clarification update:** Resolves probe/config mount strategy, immutable Hugging Face source revision policy, and local-AI fallback hierarchy. Qwen3 4B is now the preferred fallback primary model before any 0.6B-only degraded mode.
  ...
  "models": [
    {
      ...
      "privacy": "local",
      "notes": "Preferred fallback primary model when the 8B model is too heavy for the machine. Better suited to tailoring and coach features than the 0.6B triage model."
    }
  ]
}
```
...
8. If the 8B primary model is not compatible but the 4B primary model is compatible, offer a compact local setup that assigns Qwen3 4B to primary tasks and Qwen3 0.6B to triage tasks when available.
9. If the 4B primary model is also not compatible but the 0.6B triage model is compatible, do not pretend full local AI is available. Offer a triage-only local mode with clear feature limitations, or suggest cloud AI/configure-later for tailoring and coach features.
...
This is suitable for quick filtering and simple summaries. It is not recommended for CV tailoring, cover-letter quality, or interview/coach features. For those features, choose a larger local model on stronger hardware or configure a cloud provider.
```
...
Do not make local AI unavailable merely because the machine is below the 8B model's 12 GB minimum. However, do not use the 0.6B triage model as the primary model for full Hatch features.

Decision hierarchy:

1. **Balanced local setup**
   - Use Qwen3 8B for `primary` tasks.
   - Use Qwen3 0.6B for `triage` tasks.
...
3. **Triage-only local setup**
   ...
   - Disable or warn on quality-sensitive features such as CV tailoring, cover-letter generation, and interview/coach prep.
   - Use it for quick filtering, basic job summaries, smoke testing, and experimentation.
...
Example effective config for compact fallback:
...
```json
{
  ...
  "warnings": [
    "This setup uses Qwen3 0.6B for triage only. It is not suitable as the primary model for CV tailoring, cover-letter quality, or coach features."
  ]
}
```
