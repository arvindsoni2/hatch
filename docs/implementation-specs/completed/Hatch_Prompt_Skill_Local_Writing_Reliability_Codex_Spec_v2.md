---
title: Hatch Prompt Skill And Local Writing Reliability Specification v2
...
supersedes:
  - Hatch_Prompt_Skill_Local_Writing_Reliability_Codex_Spec.md
superseded_by:
...
**Status:** Implementation-ready, decisions locked  
...
The benchmark implementation and this specification must become an accepted, reviewable baseline before PR 1 begins.

1. Starting from the clean local branch through `4726aa8`, create a baseline publication PR containing:
   - the nine benchmark commits already present locally;
   - this specification under the canonical active documentation hierarchy;
   - no prompt-reliability implementation changes beyond the existing benchmark work.
2. Push the current branch or a purpose-named baseline branch to the remote.
3. Open and merge the baseline PR into the repository's normal integration branch, normally `main`, after its existing tests and documentation checks pass.
4. Tag or record the merged baseline commit in the specification header and benchmark manifest. A Git tag is optional; the exact merge commit SHA is mandatory.
5. PR 1 must branch from that merged baseline commit, not directly from an unpublished local-only commit.

The current branch name `fix/first-run-onboarding-lock-routing` does not become the long-term parent branch merely because it contains the local benchmark commits. Preserve its history through the baseline PR, then use purpose-named branches for this programme.
...
For each evidence item, store both:

- `immutable_tokens`, such as `120+`;
- `semantic_anchors`, derived from the evidence phrase, such as `locations`, `sites`, and the evidence item's stable ID.
...
3. The generated prose reproduces the associated candidate semantic claim closely enough to activate that evidence item, but drops or changes the immutable token. Activation must be deterministic and based on configured semantic anchors or an existing deterministic similarity threshold; an LLM may not be the sole judge.
...
Employer-context numbers may appear only in employer-facing or role-alignment statements and only when they occur verbatim in the normalized job description or verified employer context supplied to the task.
...
- Use batch exporting with safe shutdown flushing.
...
10. Shutdown flush is bounded and tested with a fake exporter.
...
3. The application may return structured validation issues and a non-persisted preview only if the current API already supports a safe preview. PR 1 must not add a new persisted review queue.
