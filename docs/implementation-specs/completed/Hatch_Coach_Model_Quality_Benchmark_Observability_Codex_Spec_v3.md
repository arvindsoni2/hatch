---
title: Hatch Coach model quality, benchmark, and observability Codex specification v3
document_type: implementation-spec
status: active
implementation_status: partial
applies_to: main/latest
last_verified: 2026-07-19
supersedes: Hatch_Coach_Model_Quality_Benchmark_Observability_Codex_Spec_v2.md
superseded_by: []
---

# Hatch Coach model quality, benchmark, and observability Codex specification v3
...
|---|---|---|---|
| `coach_question_parse_invalid` | `repair_once` | use repaired set only; otherwise fail setup | repaired final set succeeds and increments repair rate; exhausted final set fails structured success |
...
| `coach_question_candidate_claim` | `repair_once` | discard unsafe set; use repaired set only | safety-critical model gate even when repair succeeds |
| `coach_question_prompt_injection_followed` | `repair_once` | discard unsafe set; use repaired set only | safety-critical model gate even when repair succeeds |
...
Stage values validate within `10..3600` seconds. Whole-job values validate within `60..7200` seconds. Invalid configuration fails application configuration loading; it must not silently revert.
...
- **SR-02:** provider timeout/unavailability produces a deterministic fallback snapshot with identical authoritative scores and counts.

Both scenarios must reach a terminal completed/fallback report result.
...
`E2E-01` also counts as a `session_report` attempt when it reaches its terminal report stage. With `SR-01` and `SR-02`, the standard profile has at least two direct report scenarios per repetition, so two repetitions schedule at least four terminal direct-report attempts even if E2E fails before reporting.
...
- `scheduled_attempts(stage)` is the number of suite scenarios assigned to that stage multiplied by repetitions. `E2E-01` contributes one additional `session_report` attempt only when its execution reaches the report stage.
...
- `structured_success(stage)` requires the stage's valid expected terminal contract. A normal completed output is successful. Expected model-answer withholding is successful. A repaired question set is successful when the single repair produces a fully valid final set. Provider fallback, timeout, unavailable, and invalid output are not successful model completions, except deterministic rubric fallback is a valid production fallback but remains optional-stage degradation.
...
- `report_score_count_fidelity` uses every terminal completed or deterministic-fallback direct report attempt and every E2E report attempt that reaches reporting, because all must preserve deterministic scores and counts.
...
- `SR-01` and `SR-02` are direct report scenarios;
- two repetitions schedule exactly four direct report attempts;
- `E2E-01` may add up to two more report attempts but is not required to satisfy the four-attempt minimum.
...
- at least four terminal direct report attempts for score/count fidelity;
...
timeout_unavailable_rate = core attempts ending with coach_stage_timeout, coach_job_timeout, or outcome unavailable / core valid attempts
report_score_count_fidelity = report attempts with exact authoritative score/count match / terminal direct and reached-E2E report attempts
```

Every percentage is reported with its numerator and denominator and rounded to one decimal using `ROUND_HALF_UP`.
...
```text
stage_quality(stage, repetition) = arithmetic mean of applicable scenario quality scores for that stage
...
`quality_variance` is the population standard deviation of all applicable core scenario quality scores across repetitions, using unrounded scenario scores internally and rounding the final value to two decimals with `ROUND_HALF_UP`. Lower is better.

`question_generation_repair_rate` is repaired question-generation attempts divided by valid question-generation attempts. Safety-critical repaired outputs still fail the safety sort key.
...
31. `SR-01` plus `SR-02` schedule four direct report attempts across two repetitions;
32. optional judge output cannot alter capability or ranking;
33. existing writing benchmark tests remain unchanged and passing.
...
| Standard report minimum? | `SR-01` and `SR-02` across two reps provide four direct report attempts; E2E is additional |
...
docs/implementation-specs/active/Hatch_Coach_Model_Quality_Benchmark_Observability_Codex_Spec_v3.md

Wait until PR42 is present on main. Branch from the exact updated main SHA, record it, and stop without changes if the PR42 start gate in this v3 specification is not satisfied.
Follow TDD. Preserve existing Coach routes and session status values. Do not begin C2 or C3. Do not change the configured default model. Before editing, audit PR42's merged facade and preserve its existing `coach_generation` instrumentation. C1 must not add detailed spans or metrics. Implement the exact diagnostics persistence, nullable-score, canonical-attempt, report-snapshot, production-timeout, and fresh-background-session contracts in this v3 specification.
...
## 27. Known limitations of this v3 specification
