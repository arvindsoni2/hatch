...
const FILLER_REGEX = /\b(um|uh|er|ah|hmm|basically|literally|actually|honestly|right|like|so)\b/gi;
...
function highlightFillers(text: string): string {
  return text.replace(
    FILLER_REGEX,
    '<mark class="bg-amber-500/30 text-amber-300 rounded px-0.5">$&</mark>'
  );
}
...
export function TranscriptDisplay({ transcript, isFinal }: TranscriptDisplayProps) {
  ...
  return (
    <div className="max-h-48 overflow-y-auto rounded-xl border border-slate-700 bg-slate-800 p-4">
      <p
        className={`text-sm leading-relaxed ${isFinal ? "text-slate-200" : "text-slate-400"}`}
