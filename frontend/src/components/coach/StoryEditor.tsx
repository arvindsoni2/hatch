  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-1.5 mb-2">
        {values.map((v) => (
          <span key={v} className="flex items-center gap-1 rounded-full bg-slate-700 px-2 py-0.5 text-xs text-slate-300">
            {v}
      ...
      <div className="flex gap-2">
        <input
          ...
          className="flex-1 rounded-lg border border-slate-600 bg-slate-900 px-3 py-1.5 text-sm text-slate-200 placeholder-slate-600 focus:border-indigo-500 focus:outline-none"
        />
        <button onClick={add} className="rounded-lg border border-slate-600 bg-slate-800 px-2 hover:bg-slate-700">
  ...
  return (
    <div>
      ...
      <textarea
        ...
        className="w-full rounded-lg border border-slate-600 bg-slate-900 px-3 py-2 text-sm text-slate-200 placeholder-slate-600 focus:border-indigo-500 focus:outline-none resize-none"
      />
    </div>
  );
}
    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        <input
          ...
          className="w-full rounded-lg border border-slate-600 bg-slate-900 px-3 py-2 text-sm text-slate-200 placeholder-slate-600 focus:border-indigo-500 focus:outline-none"
        />
      </div>
      ...
      <div>
        <label className="block text-xs text-slate-400 mb-1">
          One-line summary <span className="text-slate-600">(≤200 chars)</span>
        </label>
        <input
          type="text"
          value={summary}
          ...
          className="w-full rounded-lg border border-slate-600 bg-slate-900 px-3 py-2 text-sm text-slate-200 placeholder-slate-600 focus:border-indigo-500 focus:outline-none"
        />
        <p className="text-right text-xs text-slate-600 mt-0.5">{summary.length}/200</p>
      </div>

      {/* STAR+R sections */}
      <div className="border-t border-slate-700 pt-4">
        <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-4">STAR + R</p>
      ...
      {/* Classification */}
      <div className="border-t border-slate-700 pt-4">
        <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-4">Classification</p>
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-xs text-slate-400 mb-2">Archetype fit</label>
      ...
      <div className="flex justify-end gap-3 border-t border-slate-700 pt-4">
        <Button variant="ghost" onClick={onCancel} disabled={saving} className="text-slate-400">
          Cancel
        </Button>
        <Button
          onClick={handleSave}
