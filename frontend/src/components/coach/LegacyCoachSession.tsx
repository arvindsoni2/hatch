...
export function LegacyCoachSession({ initialSession = null }: { initialSession?: SessionResponse | null }) {
  ...
  const [currentQuestion, setCurrentQuestion] = useState<SessionQuestion | null>(
    initialSession?.questions[0] ?? null,
  );
  ...
  useEffect(() => {
    ...
    if (initialSession !== null) {
      researchCompany(initialSession.company_name).then(setResearch).catch(() => {});
